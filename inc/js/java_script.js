...
var mask = 
{
	...
	function ajax_pagina(acao_adm, acao, parametro, parametro_02, parametro_03, parametro_04, parametro_05, parametro_06, parametro_07, parametro_08, div, ambiente, animar) //13
	{
		...
		url=ambiente+".php?acao_adm="+acao_adm+"&acao="+acao+"&parametro="+parametro+"&parametro_02="+parametro_02+"&_div="+div;//url da página
		var nivel = ""; 
		switch (acao_adm)
		{
...
function refresh_carrinho(row)
{
	...
	$(document).ready(function ()
	{
		...
		if($('.hidden_votos').length > 0)
		{
			$('.hidden_votos').each(function(){
				var $this = $(this);
				var resp_val = $this.attr('data-option-value');
				var votos_resp = $this.attr('value');
				
				array_data = resp_val.split('-');
				id_enquete = array_data[0];
				total_votos = array_data[1];

				var largura_p =  (votos_resp/total_votos)*100;
	
				/*$('.p_votos_'+id_enquete).animate({
				    width: largura+'%',
				    height: '2px'
				}, {
				    duration: 1000,
				    ...
				    complete: function() {
				      $(this).after('<div>'+largura+' %</div>');
				    }
				});*/
				var largura = ((largura_p/100)*(240));
				$('.p_votos_'+id_enquete).animate({
				    width: largura+'px',
				    opacity: 1,
				    fontSize: "3em",
				    borderWidth: "10px"
			    	   ...
			    	   complete: function() {
				      $(this).after('<div class="div_resultado_votos">'+largura_p.toFixed(0)+' %</div>');
				    }
				}
			     );
		...
		//Filtra as categorias da galeria virtual
		$('.link_categoria').click(function (){
			$this = $(this);
			var value = $this.attr('data-option-value');
			//$('.link_categoria').removeClass('active_02');
			
			if($this.hasClass('active_02'))
			{
				value = '*';
				$('.link_categoria').removeClass('active_02');
			}
		...
		//SELECIONA O ENQUETE
		$('.click_enquete').click(function () {
			
			$this = $(this);
			id_pergunta = $this.attr('data-option-value');
			var id_resposta = $('input:radio[name=group_'+id_pergunta+']:checked').val();
			if(id_resposta != null)
			{
				ajax_pagina('seleciona_enquete', '', '', id_pergunta, id_resposta, '', '', '', '', '', 'div_enquete', 'ajax/ajax_site', 'false');
	...
	{
		//$('.a_usr').click(function() {
			
			/*$this = $(this);
			var value = $this.attr('data-option-value');*/
		    /*$(this).next('div').slideToggle('fast')
		    .siblings('div:visible').slideUp('fast');*/
		   // $('.div_hid_'+value).siblings('div:visible').slideToggle('fast');alert('asd');
	...
	function modifica_ordem_home(id_produto, ordem)
	{
		if (id_produto > 0)
		{
			...
			success: function(html)
			{
				...
				{
					alert('Não foi possível realizar a modificação. Por favor tente novamente!');
				}
				else
				{
	...
	{
		//ajax_pagina('notificao','pedido','teste','','','','','','','','ajax_notificacoes','ajax_gc_adm','false');
		if($("#ajax_notificacoes").length)
		{
			$.ajax({
				url: "../ajax/ajax_site.php",
				data: {acao_adm: "notificao"},
				cache: false,
				success: function(html)
				{
					if(html.match(/.*novas_img.*/i))
					{
						$.growlUI('Notificação', 'Tem itens da Galeria Virtual pra ser revisados!', 5000);
					}
				}
			});
		}
