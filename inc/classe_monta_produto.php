...
class Monta_produto
{
	...
	function monta_lista_noticias()
	{
		...
		if($rs = $o_produto->selecionar_produto_complemento())
		{
			...
			if($this->acesso == "sim")
			{
				$header = "<a href=\"#\" onclick=\"fancy_function('".$url_virtual."componentes/popup_outros.php?email=".$o_configuracao->email_noticias()."&msg=Notícias&id_produto_tipo=1',700,575,'iframe')\">";
			}
			else
			{
				$header = "<a href=\"".$url_virtual."login/noticias\">";
			}
								...
								<div class=\"element_destaque col2 box_filtro\">
									<div class=\"element_contenedor_destaque\">
										...
										<p>Dividir conte&uacute;dos<br/>  interessantes tamb&eacute;m <br/> &eacute; uma forma de aprendizado.</p>
														...
														<h3 class=\"h_cero\">".$header."Ent&atilde;o divida conosco.</a></h3>
									</div>
	...
	function monta_lista_correspondentes()
	{
		...
		if($rs = $o_usuario->selecionar_usuario_complemento())
		{
			...
			if($this->acesso == "sim")
			{
				$header = "<a href=\"#\" onclick=\"fancy_function('".$url_virtual."componentes/popup_outros.php?email=".$o_configuracao->email_correspondentes()."&msg=Correspondentes&id_produto_tipo=4',700,575,'iframe')\">";
			}
			else
			{
				$header = "<a href=\"".$url_virtual."login/correspondentes\">";
			}
							...
							<div class=\"element_contenedor_2_divs_c box col3\">
								...
								<div class=\"element_destaque col3 box_filtro\">
									<div class=\"element_contenedor_destaque_c\">
										...
										<h3 class=\"h_cero\">".$header."Clique e saiba mais.</a></h3>
	...
	function monta_lista_oportunidade()
	{
		...
		if($rs = $o_produto->selecionar_produto_complemento())
		{
			...
			if($this->acesso == "sim")
			{
				$header = "<a href=\"#\" onclick=\"fancy_function('".$url_virtual."componentes/popup_outros.php?email=".$o_configuracao->email_oportunidades()."&msg=Oportunidades&id_produto_tipo=2',700,575,'iframe')\">";
			}
			else
			{
				$header = "<a href=\"".$url_virtual."login/oportunidades\">";
			}
	...
	function monta_lista_galeria()
	{
		...
		if($rs = $o_produto->selecionar_produto_complemento())
		{
			...
			{
				$header = "<a href=\"".$url_virtual."login/galeria_virtual\">";
			}
					...
					$resultado .= '
							...
							<div id="tagcloud">
								';
									...
									$o_categoria->set('ordenador', 'RAND()');
									if($rs_categoria = $o_categoria->selecionar())
									{
										foreach($rs_categoria as $l_categoria)
										{						
											/*Monta classe de acordo com links mais acessados*/
											if($ac = $o_categoria->selecionar_acessos())
											{
												foreach($ac as $linha)
												{
											...
											$qtde_acesso = explode("-",$acessos);
											
											if($l_categoria['acesso'] <= $qtde_acesso[4])
											{
												$class = "smallest";
											}
											elseif($l_categoria['acesso'] == $qtde_acesso[3])
											{
												$class = "small";
											}
											elseif($l_categoria['acesso'] == $qtde_acesso[2])
											{
												$class = "medium";
											}
											elseif($l_categoria['acesso'] == $qtde_acesso[1])
											{
												$class = "large";
											}
											elseif($l_categoria['acesso'] == $qtde_acesso[0])
											{
												$class = "largest";
											}
											/*Monta classe de acordo com links mais acessados*/
											...
											$resultado .= '<a href="#" class="link_categoria '.$class.'" data-option-value="'.$filtro_categoria.'" onclick=\'javascript:ajaxHTML("temp","'.$o_configuracao->url_virtual().'inc/busca_ajax.php?tipo=acessados&parametro='.$l_categoria['id'].'");\'>'.$l_categoria['nome'].'</a>';
										}
									}
